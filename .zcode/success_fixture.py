...
def build_corpus(root: Path) -> dict:
    ...
    ep_cols = [
        ...
        "videos/observation.images.up/chunk_index", "videos/observation.images.up/file_index",
        "videos/observation.images.up/from_timestamp", "videos/observation.images.up/to_timestamp",
        "videos/observation.images.side/chunk_index", "videos/observation.images.side/file_index",
        "videos/observation.images.side/from_timestamp", "videos/observation.images.side/to_timestamp",
        "tasks", "stats/next.success/min", "stats/next.success/max",
    ]
    rows = [
        [i, 1, "000", "000", i, i + 1,
         "000", "000", 0.0, 0.0,
         "000", "000", 0.0, 0.0,
         [f"task-{i}"], [False], [False]]
    ...
    vals = ",".join(
        "(" + ",".join(
            "[" + ",".join(f"'{x}'" for x in v) + "]" if isinstance(v, list)
    ...
    data_rows = [
        [i, i, 0, 0.0, [0.0], [0.5], False] for i in range(EPISODES)
    ]
    data_path = root / "data" / "chunk-000" / "file-000.parquet"
    data_path.parent.mkdir(parents=True, exist_ok=True)
    dvals = ",".join(
        "(" + ",".join(
            str(v) if not isinstance(v, (str, list)) else
...
def main() -> None:
    ...
    stubs = {
        ...
        "_hf_tree": lambda repo, rev, path: (
            ...
            if "episodes" in path else [{"path": "meta/info.json", "type": "file"}]
        ),
    ...
    try:
        ...
        prep.import_lerobot_dataset(
            dataset_repo="fake/repo", output_dir=output_dir,
