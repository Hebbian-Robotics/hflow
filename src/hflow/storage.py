...
def _validated_relative_key(relative: str) -> str:
    ...
    ``"C:/Windows/win.ini"`` passes this check as "relative". When
    :class:`LocalStorageRoot` later joins a validated key onto the
    real root with ``self.path / key``, the join is contained safely on
    Linux. For instance, ``Path('/data/root') / 'C:/Windows/win.ini'``
    resolves to ``'/data/root/C:/Windows/win.ini'``.
    ...
    process on a different operating system. A key produced on a Windows
    client could be validated by a Linux server. The validator should
    recognize a path as absolute under either convention, independent of
    which operating system is currently running the check. Checking
    ``PureWindowsPath.is_absolute()`` alongside the POSIX check ensures
    the guard remains universally correct.
    """
