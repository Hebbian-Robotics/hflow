...
def _validated_relative_key(relative: str) -> str:
    ...
    ``PurePosixPath.is_absolute()`` only recognizes a leading ``/``. It
    has no concept of Windows drive letters, so a key like
    ``"C:/Windows/win.ini"`` passes that check as "relative".

    This check is platform-independent because storage keys can be
    generated by a client on one operating system and validated by a
    process on a different one. A key produced on a Windows client could
    be validated by a Linux server. The validator must therefore recognize
    a path as Windows-anchored under either convention, regardless of
    which operating system is currently running the check.

    Checking ``PureWindowsPath(str(key)).drive`` (rather than
    ``.is_absolute()``) is the right predicate here. A drive-relative key
    like ``"C:foo.txt"`` has a drive component but no root, so
    ``.is_absolute()`` returns False and the key would slip through.
    ``PureWindowsPath('D:/data/root') / 'C:foo.txt'`` discards the left
    operand just as ``'C:/Windows/win.ini'`` does, so both shapes must be
    refused. Keying on ``.drive`` catches all of them: drive-absolute,
    drive-relative, and UNC paths.
    """
    key = PurePosixPath(str(relative).replace("\\", "/"))
    if not str(key) or str(key) == ".":
        raise ValueError("storage key must not be empty")
    if key.is_absolute() or PureWindowsPath(str(key)).drive or ".." in key.parts:
        raise ValueError(
