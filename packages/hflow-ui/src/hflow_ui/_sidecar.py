...
UI ever writes into a workspace.
...
def load_sidecar_state(data_root: str) -> SidecarState:
    ...
    try:
        ...
        raise SidecarError(500, f"cannot read UI state file {state_file}: {error}") from error
...
def store_sidecar_state(data_root: str, state: SidecarState) -> None:
    ...
    try:
        ...
        raise SidecarError(500, f"cannot write UI state file {state_file}: {error}") from error


def _refused(state_file: Path, problem: str) -> SidecarError:
    return SidecarError(
        500, f"corrupt UI state file {state_file}: {problem}; fix or remove the file"
    )
