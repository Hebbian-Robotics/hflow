...
after the server started -- and is cached briefly per launch.
...
_LOGGER = logging.getLogger("hflow_server.runtime")
...
def is_loopback_web_url(web_url: str | None) -> bool:
    """Whether this address resolves only on the machine running this server.
...
def client_error_detail(error: AirflowClientError, *, source: RuntimeSource) -> str:
    ...
    if source == "bundle":
        ...
        # this server from another machine reads that as its own laptop -- so the
...
def create_runtime_router(settings: ServerSettings, resolver: RuntimeResolver) -> APIRouter:
    ...
    def trigger_ingest(request: IngestRequest) -> IngestTriggerResponse:
        ...
        try:
            ...
            # so a client never rebuilds the dict itself.
