...
from hflow_server import _catalog, _connections
...
def load_pipeline_state(pipeline_spec: str | None) -> PipelineState:
    ...
    if pipeline_spec is None:
        return PipelineUnavailable(
            detail=(
                "no --pipeline configured: relaunch `hflow serve` with "
...
def create_pipeline_router(settings: ServerSettings, state: PipelineState) -> APIRouter:
