"""The <data_root>/curation/state.json sidecar: atomic writes, loud boundary parsing."""
...
def bare_api(bare_root: Path) -> TestClient:
    return TestClient(create_app(ServerSettings(data_root=str(bare_root))))
...
def _write_state_file(bare_root: Path, payload: str) -> Path:
    ...
    state_file = bare_root / "curation" / "state.json"
