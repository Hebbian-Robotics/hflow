...
2. **Standard formats at every boundary; no captive UIs.** Episodes are standard MCAP (Foxglove/Rerun open them), runs are standard Airflow DAGs (Airflow's UI shows them), and the catalog, manifests, and review exports are Parquet (DuckDB/pandas/anything reads them). The optional workspace server (`hflow serve`, shipped separately as `hflow-server`) exposes these same surfaces through a documented JSON API and can serve separately supplied UI assets; no frontend ships today. The system is extensible without touching our code.
