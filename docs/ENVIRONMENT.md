...
| `HFLOW_ENDPOINT_<ALIAS>` | Overrides or supplies an endpoint declared by an app. `<ALIAS>` is uppercased and non-alphanumeric characters become `_`; colliding names are rejected. | The endpoint literal passed to `App(endpoints=...)` remains in use. If neither source defines a required endpoint, preflight fails. | Pipeline execution | `hflow.App` at each run start; see [endpoint injection](./RUNTIME.md#the-one-rule-the-apps-data-root-is-the-runtimes-data-root) |
