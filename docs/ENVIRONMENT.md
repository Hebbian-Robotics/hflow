...
| `HFLOW_MIRROR_DIR` | Chooses the base directory for local mirrors of object-store roots. | `$XDG_CACHE_HOME/hflow/mirrors`, or the platform user cache mirrors directory (e.g. `~/Library/Caches/hflow/mirrors` on macOS, `~/.cache/hflow/mirrors` on Linux) when `XDG_CACHE_HOME` is unset. Each storage URL receives a hashed subdirectory. | Storage runtime | `hflow.storage`; see [bucket data roots](./RUNTIME.md#bucket-data-roots---data-root-gsbucketprefix) |
