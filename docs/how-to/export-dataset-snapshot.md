...
directory, so a copied delivery verifies in place. A receipt path that
is absolute, contains `..`, or carries a Windows drive letter is refused
as unreadable input (exit `2`) before any file is read — the same family
